...
6. **Rerun `setup-skills-link.ps1`** after creating or renaming a top-level skill directory so each tool receives its per-skill link. Editing an existing linked skill does not require rerunning setup.
